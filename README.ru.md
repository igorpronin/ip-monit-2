...
| Обычный режим | Страны не совпадают — возможная утечка VPN | Нет интернета |
|:---:|:---:|:---:|
| <img src="docs/screenshot-normal.png" width="258" alt="Плавающее окошко: флаг, страна, IPv4 и IPv6"> | <img src="docs/screenshot-mismatch.png" width="258" alt="Плавающее окошко: IPv6 выходит из другой страны, подсвечен оранжевым"> | <img src="docs/screenshot-offline.png" width="192" alt="Плавающее окошко: нет доступа в интернет"> |
...
- **Опциональное плавающее окошко** — маленькая полупрозрачная плашка поверх всех окон, на всех рабочих столах, даже над полноэкранными приложениями. Перетаскивается куда угодно; позиция запоминается.
...
- **Виртуальная и физическая страна** — опция меню («Отображаемая страна») переключает между страной, зарегистрированной в геобазах (MaxMind — то, что видят сайты), и физическим расположением сервера (оценка Cloudflare). См. раздел [Виртуальные локации VPN](#виртуальные-локации-vpn-зарегистрированная-и-физическая-страна) ниже.
...
Приложение делает HTTPS-запросы к публичным trace-эндпоинтам Cloudflare (`https://1.1.1.1/cdn-cgi/trace` и IPv6-аналог), чтобы узнать внешние IP, и к `https://api.country.is/<ip>` для их геолокации (результат кэшируется по IP, так что запрос уходит только при фактической смене адреса). Больше ничего: без аккаунтов, без API-ключей, без аналитики; никакие данные никуда не сохраняются и не отправляются.
...
- Параллельно опрашиваются два эндпоинта: `https://1.1.1.1/cdn-cgi/trace` (IP-литерал форсирует IPv4) и `https://[2606:4700:4700::1111]/cdn-cgi/trace` (форсирует IPv6). Каждый возвращает IP клиента. Страна для каждого IP затем определяется через `api.country.is` (MaxMind GeoLite2) с кэшем по IP; поле `loc=` Cloudflare — фолбэк.
