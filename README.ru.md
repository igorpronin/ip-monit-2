...
Индикатор — светящаяся полоска у верха экрана, цветом кодирующая текущую страну выхода (здесь сработало «зелёное» правило; без совпадения она тускло-серая):

<img src="docs/screenshot-indicator.png" width="640" alt="Индикатор светится зелёным при совпадении правила страны">
...
- **Опциональное плавающее окошко** — маленькая полупрозрачная плашка поверх всех окон, на всех рабочих столах, даже над полноэкранными приложениями. Перетаскивается куда угодно; позиция запоминается. Двойной клик по окошку открывает меню приложения (удобно, когда иконка в меню-баре скрыта из-за тесноты). «Показывать окошко» и «Поверх всех окон» — отдельные переключатели: со снятой галкой «Поверх всех окон» окошко и индикатор ведут себя как обычные окна и могут перекрываться другими.
...
- **Не показывать IP-адреса** — минимально возможное окошко: одна строка `MM {флаг} {страна} v4+v6` (две строки, если IPv4 и IPv6 выходят из разных стран). В сочетании с компактным режимом — самый маленький след на экране.
- **Выравнивание слева или справа** — при правом выравнивании колонка с флагом переезжает вправо, текст выравнивается по правому краю, а окно держит правый край на месте и растёт влево при изменении размера содержимого. Удобно, когда окошко стоит у правого края экрана.
- **Индикатор** — опциональная широкая светящаяся полоска, прижатая к верху экрана по центру: цветовой код вашего расположения, считываемый одним взглядом. В настройках индикатора («Настроить индикатор…») странам назначается один из шести цветов; если текущая страна выхода (в выбранном режиме MM/CF) совпадает с правилом, полоска светится этим цветом, иначе остаётся тускло-серой. Например: Франция и Андорра → жёлтый, Португалия и Испания → зелёный, США → синий. Полоску можно перетаскивать; пункт меню возвращает её на место.
